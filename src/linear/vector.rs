...
use num_traits::{Float, Bounded, AsPrimitive};

use crate::definitions::{Number, SignedNumber, Rotation, FloatingPoint};
...
macro_rules! impl_vec {
    ($len_:expr, $($element:tt),+) => {
        ...
        fn magnitude(&self) -> Self::Scalar where Self::Scalar: Float {
            let x = T::zero() $(+ self.$element*self.$element)+;
            x.sqrt()
        }
        fn project(&self, other: &Self) -> Self where Self::Scalar: Float {
...
impl<T: FloatingPoint> Rotation<T> for Vector3<T> 
    ...
    f64: AsPrimitive<T> {
    fn quaternion(&self) -> crate::complex::Quaternion<T> { crate::complex::Quaternion::from_euler(*self) }
